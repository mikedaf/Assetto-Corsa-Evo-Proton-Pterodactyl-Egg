...
Pterodactyl mounts volumes in a way that can cause configuration files to reset. We must build a local Docker image that creates the necessary symlinks to keep your data persistent.
    ...
    SSH into your Host Node.

    Run the following commands to build the wrapper:
...
Step 2: Import the Egg
    ...
    Download the egg-assetto-corsa-evo.json file from this repository to your local computer.

    Log into your Pterodactyl Admin Panel.

    Navigate to Nests.

    Click on the Nest where you want to add this server.

    Click Import Egg and upload the egg-assetto-corsa-evo.json file.

Step 3: Create the Server

    Create a new server in Pterodactyl using the Assetto Corsa Evo (Proton) Egg.

    In the Startup tab, fill in the required variables (Steam Username/Password, Ports).
    ...
    Start the server and wait for the files to download.
